<?php
$logPath = "/var/log/mmdvm/MMDVMPLUS-" . date("Y-m-d") . ".log";
$miIndicativo = "EA3EIZ";
...
foreach ($lines as $line) {
    ...
    if (preg_match('/received (RF|network) end of voice transmission from (\w+) to TG (\d+), ([\d\.]+) seconds, .*BER: ([\d\.]+)%/', $line, $m)) {
    $callsign = $m[2];
    $tg = $m[3];
    $duration = $m[4];
    $ber = $m[5];
...
$lineas = file('/home/pi/MMDVMHost/MMDVMPLUS.ini');
